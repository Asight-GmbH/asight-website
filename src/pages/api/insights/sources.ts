...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    return jsonResponse({ sources, totalSessions, period: `${days}daysAgo to yesterday`, lastUpdated: new Date().toISOString() });
  } catch (err) {
    return jsonResponse({ error: err instanceof Error ? err.message : 'unknown', sources: [], totalSessions: 0 }, 500);
  }
};
